...
| 14 | **Member body surface in the C# index**: eval run-001 showed the extractor can LOCATE calculation rules but not extract their internals — the index exposes complexity counts, not branch conditions or expressions. Add a bounded expression/condition surface per member (deterministic, no source text dumps) so in-process calculations are extractable like procedure ASTs are. First LLM-half eval executed manually 2026-08-16 (7/7, see evals/rule-extractor/); automated agent-in-CI harness still owed under #7. | core/index/csharp |
